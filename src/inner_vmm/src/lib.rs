...
impl Vmm {
    pub fn init() -> Result<Self, Error> {
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        let load_addr: GuestMemoryMmap = GuestMemoryMmap::from_ranges(&[
		...
		load_addr.write(CODE, GuestAddress(0x00))
	...
	pub fn load_kernel_img(&self) -> Result<(), Error> {
		...
		let mut kernel_file: File = File::open("/Users/ec2-user/repos/vmm/microvm-kernel-initramfs-hello-x86_64").unwrap();

		let loader_result = Loader::load(&self.guest_memory,
			None, // not mandatory
			&mut kernel_file,
			None
		).unwrap();
